...
TimeSeriesView::TimeSeriesView(std::shared_ptr<const TimeSeries> src, size_t start, size_t len, int lag)
    ...
    if (static_cast<int>(begin_) - value_lag_ < 0 || (begin_ + length_ - value_lag_) > source_->size()) {
        throw std::out_of_range("View window/lag exceeds data boundaries");
    }
}

int64_t TimeSeriesView::timestamp(size_t i) const { return source_->get_timestamps()[begin_ + i]; }

const double* TimeSeriesView::begin() const noexcept { return &(source_->get_values()[begin_ - value_lag_]); }

const double* TimeSeriesView::end() const noexcept { return begin() + length_; }

double TimeSeriesView::operator[](size_t i) const { return source_->get_values()[begin_ + i - value_lag_]; }
...
TimeSeries TimeSeriesView::operator+(const TimeSeriesView& other) const {
    if (!is_aligned_with(other)) throw std::runtime_error("TimeSeries not aligned");
...
bool TimeSeriesView::is_aligned_with(const TimeSeriesView& other) const {
    if (source_->get_shared_timestamps() == other.source_->get_shared_timestamps() && begin_ == other.begin_ &&
