...
double calculate_noise(int64_t target,
                      int64_t t1,
                      int64_t t2,
                      double annual_vol,
                      std::mt19937& gen) {
...
double apply_strategy(InterpolationStrategy strategy,
                      std::mt19937& gen,
                      int64_t target,
                      int64_t t1,
                      double v1,
...
vector<double> TimeSeries::partial_walk(const vector<int64_t>& target_timestamps,
                                        size_t start_idx, 
                                        size_t end_idx,
    ...
    for (size_t i = 0; i<chunk_len; i++){
        ...
        if(current_target<=(*timestamps)[0]){
            ...
            new_values[i] = apply_strategy(strategy, current_gen, current_target, (*timestamps)[data_idx], values[data_idx], (*timestamps)[data_idx + 1], values[data_idx + 1]);
        }
    }
    return new_values;
}
            ...
            futures.push_back(std::async(
                ...
                [this, &target_timestamps, start, end, strategy,seed]() {
                    return this->partial_walk(
                        target_timestamps, start, end, strategy, seed
                    );
                }
            ));
        }
...
TimeSeries TimeSeries::shift(int lags) const {
    if (lags == 0) return *this;
    if (static_cast<size_t>(std::abs(lags)) >= size()) {
        return TimeSeries(std::make_shared<vector<int64_t>>(), {});
    }

    size_t start_idx_ts = (lags > 0) ? lags : 0;
    size_t end_idx_ts = (lags > 0) ? size() : size() + lags;

    size_t start_idx_val = (lags > 0) ? 0 : -lags;
    size_t end_idx_val = (lags > 0) ? size() - lags : size();

    vector<int64_t> new_ts(timestamps->begin() + start_idx_ts, timestamps->begin() + end_idx_ts);
    vector<double> new_vals(values.begin() + start_idx_val, values.begin() + end_idx_val);

    return TimeSeries(std::move(new_ts), std::move(new_vals));
}
