...
int64_t parseIso8601ToMs(const std::string& input) {
    ...
    std::chrono::sys_days days = std::chrono::year{year} / std::chrono::month{static_cast<unsigned>(month)} /
                                 std::chrono::day{static_cast<unsigned>(day)};

    std::chrono::sys_time<std::chrono::milliseconds> tp = days + std::chrono::hours{hour} + std::chrono::minutes{min} +
                                                          std::chrono::seconds{sec} + std::chrono::milliseconds{millis};
